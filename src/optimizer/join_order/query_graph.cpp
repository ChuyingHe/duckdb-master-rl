...
void QueryGraph::EnumerateNeighbors(JoinRelationSet *node, const std::function<bool(NeighborInfo *)> &callback) {
	for (idx_t j = 0; j < node->count; j++) {
		...
		for (idx_t i = j; i < node->count; i++) {
			auto entry = info->children.find(node->relations[i]);   /*try to find(key) where here key = R3*/
			if (entry == info->children.end()) {                    /*if R3 didnt exist in any edges*/
				// node not found
				break;
			}                                                       /*if R3 exists in some edge*/
			// check if any subset of the other set is in this sets neighbors
			info = entry->second.get();                             /*second is a unique_ptr<QueryEdge>, info=QueryEdge*/
			for (auto &neighbor : info->neighbors) {                /*info->neighbors=vector<unique_ptr<NeighborInfo>>*/
				if (callback(neighbor.get())) {                     /*callback(NeighborInfo)*/
					return;
				}
			}
		}
...
vector<idx_t> QueryGraph::GetNeighbors(JoinRelationSet *node, unordered_set<idx_t> &exclusion_set) {
	...
	return neighbors;   /*R4,R5*/
}
/*R3, raw pointer of a JoinRelationSet R4*/
