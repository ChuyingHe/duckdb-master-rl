...
bool JoinOrderOptimizer::TryEmitPair(JoinRelationSet *left, JoinRelationSet *right, NeighborInfo *info) {
	...
	EmitPair(left, right, info);/*R3, R4, connection*/
	return true;
}

bool JoinOrderOptimizer::EmitCSG(JoinRelationSet *node) {   /* *node = start_node, can be one relation / several relations */
	// create the exclusion set as everything inside the subgraph AND anything with members BELOW it
	unordered_set<idx_t> exclusion_set;
	for (idx_t i = 0; i < node->relations[0]; i++) {   /*start with R5, then R4, then R3, lets take node = R3 */
		exclusion_set.insert(i);
	}
	UpdateExclusionSet(node, exclusion_set);        /*if the node=R3R4R5, we need to put all three in the exclusion_set*/
	// find the neighbors given this exclusion set
	auto neighbors = query_graph.GetNeighbors(node, exclusion_set); /*...according to the Query Graph: node=R3, exclusion_set=R1,R2,R3, neighbour = R4,R5*/
	if (neighbors.empty()) {
		return true;                                    /*return true if the node R3 is isolated, no neighbors in Query Graph*/
	}
	// we iterate over the neighbors ordered by their first node
	sort(neighbors.begin(), neighbors.end());
	for (auto neighbor : neighbors) {                   /*R4,R5*/
		...
		auto neighbor_relation = set_manager.GetJoinRelation(neighbor); /*neighbor=R4, neighbor_relation = raw pointer of a JoinRelationSet R4 */
		auto connection = query_graph.GetConnection(node, neighbor_relation);
		if (connection) {                                               /*if R3 and R4 is connected*/
			if (!TryEmitPair(node, neighbor_relation, connection)) {    /*(node=R3, neighbor_relation=raw pointer of a JoinRelationSet R4, connection*/
				return false;
			}
		}
...
bool JoinOrderOptimizer::SolveJoinOrderExactly() {
	...
	for (idx_t i = relations.size(); i > 0; i--) {  /*1a.sql: 5 relations*/
		// for every node in the set, we consider it as the start node once
		auto start_node = set_manager.GetJoinRelation(i - 1);   /*start from the elem which has the biggest index*/
		// emit the start node
		if (!EmitCSG(start_node)) { /*if EmitCSG = False, then its impossible to solve Join-Oder, the node has no edge to the orders*/
			return false;
		}
